  ...
  return (
    <div className="flex flex-col overflow-hidden rounded-2xl border border-orange-100 bg-white shadow-sm transition hover:shadow-md">
      {product.photo ? (
        ...
        <img src={withBasePath(product.photo)} alt={product.name} className="h-36 w-full object-cover" />
      ) : (
        <div className="flex h-36 w-full items-center justify-center bg-orange-50 text-5xl">
          {product.image}
        </div>
      )}
