    ...
    <div className="flex flex-col overflow-hidden rounded-2xl border border-terracotta-100 bg-white shadow-sm transition hover:shadow-md">
      ...
      <div className="flex flex-1 flex-col p-3">
        ...
        <div className="mt-3 flex items-center justify-between">
          <span className="text-base font-semibold text-terracotta-700">{formatPrice(product.price)}</span>
          {quantity === 0 ? (
            <button
              onClick={() => addItem(product.id)}
              className="rounded-full bg-terracotta-600 px-4 py-1.5 text-sm font-medium text-white transition hover:bg-terracotta-700 active:scale-95"
            >
              Ajouter
            </button>
          ) : (
            <div className="flex items-center gap-2 rounded-full bg-terracotta-50 px-1.5 py-1">
              <button
                ...
                className="flex h-6 w-6 items-center justify-center rounded-full bg-white text-terracotta-700 shadow-sm active:scale-95"
              >
                −
              </button>
